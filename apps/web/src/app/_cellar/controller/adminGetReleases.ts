...
const adminGetReleases = adminProcedure
  ...
  .query(async ({ input }) => {
    ...
    */
    /*
      Repairs lines the merge bug wrote without their identity, and returns the
      open-bottle counts the repack check needs. Shared with the member's view,
      which was showing NV where this one showed the real vintage.
    */
    const openByStock = await backfillReleaseItems(items);
        ...
        const suggested = await computeReleaseQuote(
          ...
          items.map((item) => ({
            ...
            openBottles: item.stockId
              ? (openByStock.get(item.stockId) ?? 0)
              : 0,
            costPerBottle:
