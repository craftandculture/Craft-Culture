...
const SupplierUpload = () => {
  ...
  const [uploadResult, setUploadResult] = useState<{ total: number } | null>(null);
  const [uploadError, setUploadError] = useState('');

  const { data: existingWines, refetch: refetchWines } = useQuery({
    ...api.agents.getSupplierWines.queryOptions({ limit: 500 }),
  });
      ...
      reader.onload = (evt) => {
        try {
          ...
          for (const row of normalizedJson) {
            ...
            // Skip section headers with no useful data
            const hasCost = getNum([
            ...
            const hasCode = get(['Product Code', 'SKU', 'Code', 'Item Code']);

            if (!hasCost && !hasVintage && !hasCountry && !hasCode) continue;
  ...
  const handleUpload = async () => {
    if (!partnerName.trim() || parsedRows.length === 0) return;
    ...
    try {
      for (let i = 0; i < chunks.length; i++) {
        await trpcClient.agents.uploadSupplierList.mutate({
        ...
        setUploadProgress({
          current: Math.min(totalUploaded, parsedRows.length),
          total: parsedRows.length,
        });
      }

      setUploadResult({ total: totalUploaded });
    ...
    <div className="space-y-4">
      ...
      <Card>
        <CardContent className="space-y-4 p-6">
          {/* Partner name */}
          <div>
            <label className="mb-1 block text-sm font-medium">Supplier / Partner Name</label>
            <input
              type="text"
              value={partnerName}
              onChange={(e) => setPartnerName(e.target.value)}
              placeholder="e.g. Berry Bros, Justerini & Brooks, Corney & Barrow"
                  ...
                  <tbody>
                    {parsedRows.slice(0, 20).map((row, i) => (
                      <tr key={i} className="border-t border-border-muted">
                        ...
                        <td className="px-2 py-1.5 text-right">{row.moq ?? '-'}</td>
                        <td className="px-2 py-1.5 text-right">
                          {row.costPriceGbp
                            ? `${row.costPriceGbp.toFixed(2)} GBP`
                            : row.costPriceEur
                              ? `${row.costPriceEur.toFixed(2)} EUR`
                              : row.costPriceUsd
                                ? `${row.costPriceUsd.toFixed(2)} USD`
                                : '-'}
                        </td>
                      </tr>
                    ))}
          ...
          <Button
            ...
            disabled={!partnerName.trim() || parsedRows.length === 0 || !!uploadProgress}
            className="w-full"
          >
          ...
          {uploadResult && (
            <Typography variant="bodySm" className="text-green-600">
              Uploaded {uploadResult.total} wines successfully!
            </Typography>
          )}
