...
const localInventorySheetRouter = createTRPCRouter({
  ...
  get: adminProcedure.query(async () => {
    const sheetId = await settingsGetController({ key: 'localInventorySheetId' });
    const sheetName = await settingsGetController({ key: 'localInventorySheetName' });
    const lastSync = await settingsGetController({ key: 'localInventoryLastSync' });
    return {
