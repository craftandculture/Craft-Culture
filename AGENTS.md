...
### Recently Completed
- **Correcting a mis-recorded pack (Sep 2026)** — "Fix pack" on the Stock Explorer row. A repack conserves bottles because bottles physically move; a pack recorded wrongly conserves CASES, because the boxes on the shelf were never touched. The two were being confused, and only repack existed. Re-keys the LWIN-18, merges into an existing line, reissues case labels (**the physical ones need reprinting**) and records an `adjust` movement. See "Correcting a Mis-Recorded Pack" in `_wms/WMS.md`.
- **Partner merge could lose rows, and was one-way (Sep 2026)** — the uniqueness check read `information_schema.table_constraints`, which has no row for a bare `uniqueIndex()`, so a merge onto a composite key failed outright (372 rows in `wms_pricing_releases`); and any clash deleted EVERY row the duplicate held in that table, including ones the survivor lacked. Now reads `pg_index` and drops only genuinely clashing rows. Merging also renamed the retired record so it stopped grouping with its survivor — anything later attached to it could never be swept up. `_partners/utils/partnerReferenceColumns.ts` is now the one place the FKs onto `partners` are discovered.
- **An order edited in Zoho no longer strands (Aug 2026)** — release is gated on `zohoStatus === 'invoiced'`, but the sync short-circuited on Zoho's last-modified time, which an invoice edit does not reliably bump. Status is compared too, so an order that becomes invoiced returns to Ready for Release on its own. Note the flip side: editing an invoice in Zoho legitimately pulls the order off the pick list until it is reissued **from the sales order**.
- **Client LPO → draft Zoho order (Aug 2026)** — `/platform/admin/lpo` reads a client's purchase order PDF *or* a replenishment spreadsheet, matches every line to the catalogue, checks price against the last published quote, and creates a **draft** Zoho sales order with any item codes it needs. See `_lpo/LPO.md` for the five Zoho rules that each cost a wrong order to learn — chiefly that the SKU is the pack being SOLD, not the stock held, and that Zoho enforces unique item names rather than SKUs.
